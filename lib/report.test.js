...
import { buildReport, buildRecentItems, buildRows } from "./report";
...
describe("buildReport", () => {
  ...
  it("buildRows คืนแถวราบทุกแถว (รวม orphan) ใหม่สุดก่อน", () => {
    const rows = buildRows({
      attempts: [
        at("B1", "20", true, "2026-08-20T09:00:00Z"),
        at("B1", "40", false, "2026-08-20T10:00:00Z", { session_key: "s1" }),
      ],
      problemsById: problems,
      transcriptsBySession: {
        s1: [{ seq: 0, role: "student", text: "40" }],
        s9: [
          { seq: 0, role: "question", text: "งง", problem_id: "B1", created_at: "2026-08-20T11:00:00Z" },
        ],
      },
    });
    expect(rows).toHaveLength(3); // ถูก + ผิด + orphan
    expect(rows[0].orphan).toBe(true); // ใหม่สุดก่อน
    expect(rows[1].answer).toBe("40");
    expect(rows[2].isCorrect).toBe(true);
  });

  it("buildRecentItems ตัดด้วย since — กดอ่านแล้วรายการหายจากจอ (ข้อมูลยังอยู่)", () => {
    const args = {
      attempts: [
        at("B1", "20", true, "2026-08-01T10:00:00Z"),
        at("B1", "40", false, "2026-08-25T10:00:00Z"),
      ],
      problemsById: problems,
      transcriptsBySession: {},
    };
    expect(buildRecentItems({ ...args, since: null })).toHaveLength(2);
    const recent = buildRecentItems({ ...args, since: "2026-08-20T00:00:00Z" });
    expect(recent).toHaveLength(1);
    expect(recent[0].answer).toBe("40");
  });

  it("session ที่ถามอย่างเดียวไม่กดส่ง → โผล่เป็น orphan ครูเห็น ไม่หาย", () => {
    const r = buildReport({
      attempts: [],
      problemsById: problems,
      transcriptsBySession: {
        s9: [
          { seq: 0, role: "question", text: "ข้อนี้เริ่มยังไง", problem_id: "B1", created_at: "2026-08-20T10:00:00Z", seconds_on_problem: 120 },
          { seq: 1, role: "hint", text: "ลองวาดแรงก่อน", problem_id: "B1", created_at: "2026-08-20T10:01:00Z", seconds_on_problem: 150 },
        ],
      },
    });
    expect(r.all.totals.attempts).toBe(0);
    expect(r.all.totals.orphanSessions).toBe(1);
    expect(r.all.totals.questions).toBe(1);
    expect(r.all.byTag).toHaveLength(1);
    expect(r.all.byTag[0].matched).toBe(false);
    expect(r.all.byTag[0].items[0].orphan).toBe(true);
    expect(r.all.byTag[0].items[0].problemId).toBe("B1");
    expect(r.all.recentQuestions).toHaveLength(1);
    expect(r.all.recentQuestions[0].text).toContain("เริ่มยังไง");
    expect(r.all.recentQuestions[0].reply).toContain("วาดแรง");
  });

  it("orphan ไม่นับรวมในแกนหัวข้อ กัน %ผิดเพี้ยน", () => {
    const r = buildReport({
      attempts: [at("B1", "20", true, "2026-08-20T09:00:00Z", { session_key: "s1" })],
      problemsById: problems,
      transcriptsBySession: {
        s1: [],
        s9: [
          { seq: 0, role: "question", text: "งง", problem_id: "B1", created_at: "2026-08-20T10:00:00Z" },
        ],
      },
    });
    // มีแต่ตอบถูก + orphan → byTopic ว่าง (ไม่มีผิดจากการกดส่งจริง)
    expect(r.all.byTopic).toEqual([]);
    expect(r.all.totals).toMatchObject({ attempts: 1, correct: 1, wrong: 0, orphanSessions: 1 });
  });
});
