...
describe("buildReport", () => {
  ...
  const sheet = (strokes = [[[0.1, 0.2]]]) => ({
    strokes: strokes.map((p) => ({ c: "#1f2933", w: 0.0035, p })),
    aspect: 1.4,
    layout: { imageBox: [0, 0.3, 1, 0.5] },
  });

  it("ลายมือผูกเข้ากับการตอบผ่าน session_key และส่ง URL รูปไปให้วาดพื้นหลัง", () => {
    const r = buildReport({
      attempts: [at("B1", "40", false, "2026-08-20T10:00:00Z", { session_key: "s1" })],
      problemsById: problems,
      scratchBySession: { s1: sheet() },
    });
    const item = r.all.byTag[0].items[0];
    expect(item.scratch.aspect).toBe(1.4);
    expect(item.scratch.layout.imageBox).toEqual([0, 0.3, 1, 0.5]);
    expect(item.image).toBe("/problems-images/b1.png");
  });

  it("แผ่นเปล่า (เปิดโจทย์แต่ไม่ได้เขียน) → ถือว่าไม่มีลายมือ ไม่ต้องโชว์ปุ่มเปล่า", () => {
    const r = buildReport({
      attempts: [at("B1", "40", false, "2026-08-20T10:00:00Z", { session_key: "s1" })],
      problemsById: problems,
      scratchBySession: { s1: { strokes: [], aspect: 1 } },
    });
    expect(r.all.byTag[0].items[0].scratch).toBe(null);
    expect(r.all.totals.scratched).toBe(0);
  });

  it("นับจำนวนข้อที่เขียนทด โดยไม่นับซ้ำเมื่อโจทย์ข้อเดียวตอบหลายครั้ง", () => {
    const r = buildReport({
      attempts: [
        at("B1", "40", false, "2026-08-20T10:00:00Z", { session_key: "s1" }),
        at("B1", "50", false, "2026-08-20T10:05:00Z", { session_key: "s1" }),
        at("A1", "180", false, "2026-08-20T11:00:00Z", { session_key: "s2" }),
      ],
      problemsById: problems,
      scratchBySession: { s1: sheet(), s2: sheet() },
    });
    expect(r.all.totals.scratched).toBe(2); // ไม่ใช่ 3
  });

  it("ไม่มีลายมือเลย → เป็น null และนับเป็น 0 ไม่พัง", () => {
    const r = build([at("B1", "40", false, "2026-08-20T10:00:00Z")]);
    expect(r.all.byTag[0].items[0].scratch).toBe(null);
    expect(r.all.totals.scratched).toBe(0);
  });

  it("โจทย์ที่ไม่มีรูป → image เป็น null (หน้าครูจะวาดแค่ลายมือบนพื้นขาว)", () => {
    const r = build([at("A1", "180", false, "2026-08-20T10:00:00Z")]);
    expect(r.all.byTag[0].items[0].image).toBe(null);
  });
});
