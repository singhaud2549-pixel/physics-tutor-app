...
// รายการราบใหม่สุดก่อน ไว้โชว์เป็นข้อๆ (กดอ่านแล้ว = ขยับ since → รายการหายจากจอนี้ แต่ข้อมูลยังอยู่ใน DB)
export function buildRecentItems({
  ...
  const rows = buildRows({ attempts, problemsById, transcriptsBySession });
  return since ? rows.filter((r) => r.createdAt && r.createdAt > since) : rows;
}
...
export function buildReport({
  ...
  const displayRows = buildRows({ attempts, problemsById, transcriptsBySession });
