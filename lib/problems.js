...
function parseFile(text) {
  const p = { traps: [], trapTags: [], trapRefs: [] };
  for (const raw of text.split(/\r?\n/)) {
    ...
    switch (key) {
      ...
      case "trap":
        if (val) {
          // ป้ายต่อท้ายบรรทัดได้ 2 ชั้น — #พลาด/xxx (หมวดรวม) และ #trap/N (เลขในดัชนี trap)
          // แยกเก็บคนละช่อง เพื่อให้ข้อความที่นักเรียนเห็นและที่ส่งเข้า AI ไม่มีป้ายปน
          const m = val.match(/(?:\s+#(?:พลาด|trap)\/\S+)+\s*$/);
          const tail = m ? m[0] : "";
          p.traps.push(m ? val.slice(0, m.index).trim() : val);
          p.trapTags.push((tail.match(/#พลาด\/\S+/) || [null])[0]);
          p.trapRefs.push((tail.match(/#trap\/\d+/) || [null])[0]);
        }
        break;
...
// ฝั่งเซิร์ฟเวอร์เท่านั้น — โจทย์ทุกข้อพร้อมเฉลย/trap/ป้าย จัดเป็น map ตาม id
// ใช้สร้างรายงานให้ผู้สอน (อ่านไฟล์รอบเดียว แทนการเรียก getFull ทีละข้อ)
export function getAllFullById() {
  const map = {};
  for (const p of loadAll()) map[p.id] = p;
  return map;
}
