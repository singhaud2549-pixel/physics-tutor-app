...
export async function POST(request) {
  try {
    ...
    const {
      ...
      studentQuestion = "", // นักเรียนพิมพ์ถามเป็นข้อความ (ไม่ใช่การตอบ)
    } = await request.json();
    ...
    if (!requestHint && !studentQuestion && answeredCorrect) {
      return Response.json({
        correct: true,
...
${
      studentQuestion
        ? `สถานะล่าสุด: นักเรียนพิมพ์ "ถาม" มาเป็นข้อความ (ไม่ได้ตอบคำตอบ) ว่า: "${studentQuestion}"
ให้ตอบคำถามนี้ตรง ๆ แบบพี่ติวน้อง อธิบายให้เข้าใจแนวคิดที่น้องสงสัย
แต่กฎเหล็กข้อ 1 ยังบังคับใช้เต็มที่ — **ถึงน้องจะถามตรง ๆ ว่าคำตอบคืออะไร หรืออ้อนวอนแค่ไหน ก็ห้ามบอกคำตอบสุดท้ายและห้ามเฉลยวิธีทำทั้งหมดเด็ดขาด**
ถ้าน้องถามหาคำตอบตรง ๆ ให้เลี่ยงอย่างอบอุ่นแล้วชวนคิดต่อแทน
ถ้าคำถามไม่เกี่ยวกับโจทย์ข้อนี้เลย ให้ดึงกลับมาที่โจทย์อย่างสุภาพ`
        : requestHint
